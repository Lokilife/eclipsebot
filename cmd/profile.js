...
module.exports = {
    run: (bot,message,args,con)=> {try{
        ...
        else return message.channel.send(con.defEmb.setTitle(argsUser.username).addField('Дата регистрации:', `${strftime('%B %d, %Y год в %H:%M', date2)}\n(${diff1} дней назад)`,true).addField('Подключился на сервер:', `${strftime('%B %d, %Y год в %H:%M', date3)}\n(${diff2} дней назад)`,true).addField(`ID:`,`${argsUser.id}`,true).addField(`Имеет статус:`, `\`\`\`${statuses[argsUser.presence.status]}\`\`\``).setThumbnail(argsUser.avatarURL({ dynamic: true })|| argsUser.defaultAvatarURL).setFooter(con.footer))
    
        if(game == `\`\`\`Не в сети\`\`\``) return message.channel.send(con.defEmb.setTitle(argsUser.username).addField('Дата регистрации:', `${strftime('%B %d, %Y год в %H:%M', date2)}\n(${diff1} дней назад)`,true).addField('Подключился на сервер:', `${strftime('%B %d, %Y год в %H:%M', date3)}\n(${diff2} дней назад)`,true).addField(`ID:`,`${argsUser.id}`,true).addField(`Имеет статус:`, `\`\`\`Не в сети\`\`\``).setThumbnail(argsUser.avatarURL({ dynamic: true })|| argsUser.defaultAvatarURL).setFooter(con.footer))
